...
- **Major events:** dedicated public-page readers track the Oxford Farming Conference, Oxford Real Farming Conference, Marmalade Festival during Skoll World Forum week, Oxford Literary Festival, IF Oxford Science and Ideas Festival, and Oxford International Song Festival. Multi-day dates count on each day and export as one calendar event spanning the published range. The Skoll Foundation site disallows this crawler, so the app uses Marmalade's public partner page as the Forum-week signal; check the [official Skoll page](https://skoll.org/skoll-world-forum/) manually for Forum details. The Marmalade page does not establish attendance at the Forum itself.
- **Oxford Symposia:** the organiser's [upcoming symposiums](https://www.oxfordsymposia.co.uk/event-list) are read from public detail pages that give full date ranges and named Oxford venues. These are independent events and should not be assumed to be University of Oxford events simply because they meet in a college or the Oxford Union.
- **Directory fallback:** [Conal Conference Alerts' Oxford listings](https://conferencealerts.com/city-listing?city=Oxford) are public and readable by the app. They are marked low confidence with an unverified venue because a city listing does not prove an in-person gathering at a particular Oxford venue. Check the organiser's page before relying on one. A higher-confidence organiser event keeps its details if the same title and date also appear in the directory.
- **Calendar links:** source scans identify public iCal/ICS feeds and individual event downloads. The Sources page shows feed links for Apple Calendar and an **Import feed** button when a feed has not already been added. The [Oxford Modern Languages events feed](https://www.mod-langs.ox.ac.uk/events/ical.ics) is included as an example and is rescanned like other sources. An individual "Add to my calendar" download is shown as a count, not mistaken for a whole-site subscription. Calendar imports are limited to future events and retain source links for review.
- **Review and exports:** filter by date, venue, type, confidence and status; approve or ignore leads without a page reload. Tick individual events or Select all shown events to change several review statuses at once. Export CSV or an all-day ICS calendar. The page records source URL and when it was last checked. Rescans flag changed details and return a changed approved item to New.
- **Trade-focused diary:** individual lectures, seminars, webinars, routine dinners and receptions, exhibitions and small tours are left out of the diary and exports. Existing stored leads of these kinds are hidden automatically after an update. Conferences, symposiums, workshops, graduations, open days, major public events, performances and home matches remain in view.
...
- **AllConferenceAlert Oxford:** the dashboard links to its [Oxford listings](https://www.allconferencealert.com/oxford) for manual checks. On 16 September 2026 its robots.txt permitted the page, but the site returned HTTP 403 to this app and the public web reader. The app does not retry through another identity or route around that refusal. Verify any promising listing with its organiser before adding it to the pub diary.
...
The registry starts with Oxford Events (the University’s current central listing), Oxford colleges, departments and institutes, city listings, nearby theatres, Oxford United, major conference and festival pages, Oxford Brookes graduation, Oxford Pride, May Morning and other public pages. It is a starting list, not a guarantee that every URL is a working calendar. You can edit URLs, enable or disable sources and choose a reader on the **Sources** page. Choose **Ical** for a public `.ics` feed. New seed sources are added to an existing database on upgrade without replacing edited entries. Annual event pages may need a new public URL when the organiser replaces a year-specific page; an **empty** or **error** health status tells you when to review one.
...
The crawler checks robots.txt before fetching a page, uses a descriptive user agent, spaces requests per host, caps pages and reports errors. It does not log in, solve CAPTCHAs, bypass paywalls or attempt to discover private bookings. If robots.txt returns a 4xx status other than 429, the app treats the rules file as unavailable under [RFC 9309](https://www.rfc-editor.org/rfc/rfc9309.html#section-2.3.1.3) and tries the public page once using its normal identity. A page-level 403 remains an error and is never bypassed. A robots 429, 5xx or network failure stops the request. Site terms may impose further restrictions even when robots.txt allows access: check terms before enabling a source and disable any source that prohibits this use. Avoid reposting event descriptions or using this tool as a substitute for organisers' listings.
